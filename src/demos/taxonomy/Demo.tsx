...
import { CASES, stateFor } from "./examples"
...
import type { AnswerSource, ChoiceAnswer, JevUsage, JevWire } from "@shared/jev.ts"

const demo = demoBySlug("taxonomy")!
const MAX_DEPTH = 3
...
export default function TaxonomyDemo() {
  ...
  const item = CASES.find((entry) => entry.id === selected)!
    ...
    try {
      for (let depth = 0; depth < MAX_DEPTH; depth += 1) {
        ...
        const response = await decide({
          ...
          fixtureKey: `taxonomy/${item.id}-d${depth}`,
        })
        requests += 1
        ...
        rounds.push(branches)
      }

      setRun({
        rounds,
        winner: best(branches),
        usage,
        requests,
  ...
  return (
    <DemoFrame demo={demo}>
      <RunBar
        examples={CASES.map((entry) => ({ id: entry.id, label: entry.label }))}
        selected={selected}
        onSelect={(id) => {
          setSelected(id)
          setRun(null)
      ...
      <Card>
        <div className="p-4 sm:p-5">
          ...
          <p className="mt-2 text-[11px] text-ink-muted">
            Filed by hand as{" "}
            <span className="font-mono text-ink-secondary">{item.expected}</span>,
            before any run.
          </p>
        </div>
      </Card>
      ...
      {run ? <Outcome run={run} /> : null}
      {run ? <Tree run={run} /> : null}
      ...
      <div className="flex flex-wrap items-center gap-x-6 gap-y-3 p-4 sm:p-5">
        ...
        {run.source === "live" ? (
          <div className="ml-auto">
            <UsageReadout usage={run.usage} calls={run.requests} />
          </div>
        ) : null}
      </div>
              ...
              {[...branches]
                ...
                .map((branch) => (
                  <BranchRow
                    ...
                    branch={branch}
                    isWinner={
                      run.winner?.path.join("/") === branch.path.join("/")
                    }
                  />
                ))}
            </ul>
